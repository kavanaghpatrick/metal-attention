...
fn main() {
    ...
    for metal_file in &metal_files {
        ...
        // Debug symbols for debug builds, optimization for release
        if profile == "release" {
            cmd.arg("-O2");
        } else {
            cmd.args(["-gline-tables-only", "-frecord-sources"]);
        }
    ...
    let output = cmd.output().expect("Failed to run xcrun metallib linker");
