...
// ---------------------------------------------------------------------------
// Q4_0 matvec with accumulate: output[row] += dot(weight_row, input)
//
// Fuses matvec + residual_add_inplace into a single dispatch.
// Safe because each row writes to a unique output index (no races).
//
...
kernel void matvec_q4_0_accumulate(
    device const BlockQ4_0* weight  [[buffer(0)]],
    device const float*     input   [[buffer(1)]],
    device float*           output  [[buffer(2)]],  // READ-MODIFY-WRITE
    ...
    if (simd_lane == 0) {
        output[row] += sum;  // ACCUMULATE instead of overwrite
    }
}
